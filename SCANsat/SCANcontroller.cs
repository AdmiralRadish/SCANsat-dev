...
namespace SCANsat
{
	...
	public class SCANcontroller : ScenarioModule
	{
		...
		private Dictionary<CelestialBody, int> scaledSpaceMapSourceIds = new Dictionary<CelestialBody, int>();
		private CelestialBody bigMapBodyScaledSpace;
		private CelestialBody zoomMapBodyScaledSpace;
		private int textureRefreshFrameCounter;
		...
		private void Update()
		{
			...
			// Periodically check if the source texture on the material has been
			// swapped (e.g. Parallax / Sol loading a higher-quality map after
			// initial scene setup). Only check every 30 frames to keep it cheap.
			textureRefreshFrameCounter++;
			if (textureRefreshFrameCounter >= 30)
			{
				textureRefreshFrameCounter = 0;
				RefreshScaledSpaceTexturesIfChanged();
			}
		}
		...
		void GetVisualMapTexturesForBody(CelestialBody b, out Material material, out bool useMaterialForColorMap, out string colorMapTextureName, out string normalMapTextureName)
		{
			...
			else if (shaderName.Contains("HapkeScaled") || SCANparallaxContinued.IsParallaxBody(b))
			{
				// HapkeScaled (from AdvancedPQSTools / newer Parallax) replaces the
				// standard shader on the scaledBody. Its _ColorMap already contains valid
				// planet texture data. Do NOT call LoadParallax().Load() here — that
				// resets the texture when the Parallax scaled manager is inactive
				// (e.g. in SpaceCenter), producing a white map.
				// The _BumpMap on HapkeScaled bodies often contains a blank placeholder
				// whose constant blue channel washes the visual map white via the
				// soft-light blending in SCANmap; always skip it.
				useMaterialForColorMap = false;
		...
		void CacheScaledSpaceTexture(Dictionary<CelestialBody, Texture2D> cache, CelestialBody b, Material material, string textureName, bool useMaterial, bool forceOpaque = false)
		{
			try
			{
				if (textureName == null)
				...
				// Only for the color map cache: track source texture identity so we
				// can detect when Parallax/Sol swaps in a higher-quality version.
				if (cache == readableScaledSpaceMaps)
				{
					if (scaledSpaceMapSourceIds.TryGetValue(b, out int cachedId) && cachedId == sourceId && cache.ContainsKey(b) && cache[b] != null)
						return; // same source texture, cache is still valid

					scaledSpaceMapSourceIds[b] = sourceId;
				}
				else
				{
					if (cache.ContainsKey(b) && cache[b] != null)
						return;
				}

				Texture2D colorMap;

				// Try a direct CPU-side pixel copy first. This preserves exact RGB
				// values and avoids RenderTexture blending / color-space artifacts
				// that corrupt low-alpha pixels (e.g. HapkeScaled ocean-mask alpha).
				if (sourceTexture.isReadable)
				{
					var pixels = sourceTexture.GetPixels32();
					colorMap = new Texture2D(sourceTexture.width, sourceTexture.height, TextureFormat.RGBA32, false);
					if (forceOpaque)
					{
				...
				{
					// Source is not readable — must go through GPU Blit.
					colorMap = readableTexture(sourceTexture, useMaterial ? material : null);
					if (forceOpaque && colorMap != null)
					{
						var pixels = colorMap.GetPixels32();
						for (int i = 0; i < pixels.Length; i++)
							pixels[i].a = 255;
						colorMap.SetPixels32(pixels);
						colorMap.Apply();
				...
				// Destroy the old cached texture if we're replacing it
				if (cache.ContainsKey(b) && cache[b] != null)
				{
					Log.Debug($"Re-caching scaled space texture for {b.name} (source texture changed)");
					GameObject.Destroy(cache[b]);
				}
		...
		private void RefreshScaledSpaceTexturesIfChanged()
		{
			if (!SCAN_Settings_Config.Instance.VisibleMapsActive)
				return;

			bool refreshedBigMap = false;
			bool refreshedZoomMap = false;

			if (bigMapBodyScaledSpace != null)
				refreshedBigMap = RefreshBodyTextureIfChanged(bigMapBodyScaledSpace);

			if (zoomMapBodyScaledSpace != null && zoomMapBodyScaledSpace != bigMapBodyScaledSpace)
				refreshedZoomMap = RefreshBodyTextureIfChanged(zoomMapBodyScaledSpace);
		...
		private bool RefreshBodyTextureIfChanged(CelestialBody b)
		{
			...
			if (material == null || colorMapTextureName == null)
				return false;

			var sourceTexture = material.GetTexture(colorMapTextureName) as Texture2D;
			if (sourceTexture == null)
				return false;

			int currentId = sourceTexture.GetInstanceID();

			// If we have a cached source ID and it matches, the cache is still valid
			if (scaledSpaceMapSourceIds.TryGetValue(b, out int cachedId) && cachedId == currentId)
				return false;

			// Either the source texture changed, or the initial cache failed and
			// the texture has now become available (e.g. AdvancedPQSTools loaded it
			// after scene setup). Cache it now.
			bool isFirstCache = !readableScaledSpaceMaps.ContainsKey(b) || readableScaledSpaceMaps[b] == null;
			Log.Debug($"{(isFirstCache ? "Late-caching" : "Re-caching")} visual map texture for {b.name}");
			CacheScaledSpaceTexture(readableScaledSpaceMaps, b, material, colorMapTextureName, useMaterialForColorMap, forceOpaque: true);
			return true;
		}
		...
		internal void UnloadVisualMapTexture(CelestialBody b, mapSource s)
		{
			...
			scaledSpaceMapSourceIds.Remove(b);
